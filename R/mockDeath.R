...
#'            It is essential that the 'person' and 'observation_period' tables are populated as they provide necessary context for generating death records.
#'
#' @param recordPerson An integer specifying the expected number of death records to generate per person.
...
#'                     assuming most datasets would only record a single death date per individual if at all.
#'
...
#'             If 'NULL', the seed is not set, which can result in different outputs on each run.
#'
#' @return Returns the modified `cdm` object with the new 'death' table added. This table includes the simulated
